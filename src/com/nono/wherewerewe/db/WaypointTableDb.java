...
public class WaypointTableDb {
	...
	public static final String CREATE_TABLE_CMD = 
	      "create table " + TABLE_NAME + " (" +
	        	...
	        	DbConst.TRAIL + " integer, " +
	        	DbConst.LAST_CRUMB + " integer, " +
	        	DbConst.CRUMB_NUM + " integer, " +
	      ");";
	public static final String DROP_TABLE_CMD = "DROP TABLE IF EXISTS " + TABLE_NAME + ";";

	private final DbAdapter dbAdapter;
	
	public WaypointTableDb(DbAdapter dbAdapter) {
    ...
    public WaypointTable[] getWaypointList(long tripId) {
		...
		Cursor cursor =
        	dbAdapter.getSqldb().query(TABLE_NAME, new String[] {DbConst.KEY_ID, DbConst.TRIP_ID, DbConst.NAME,
            		DbConst.WHEN, DbConst.ADDRESS, DbConst.ALTITUDE, DbConst.LATITUDE, DbConst.LONGITUDE, DbConst.COMMENT, DbConst.TRAIL, DbConst.LAST_CRUMB, DbConst.CRUMB_NUM},
            		sb.toString(), null, null, null, DbConst.WHEN);
        if (cursor == null) {
        ...
        while (cursor.moveToNext()) {
	        ...
	        table.setTrail(cursor.getLong(columnIndex++));
	        table.setLastCrumb(cursor.getInt(columnIndex++));
	        table.setCrumbNum(cursor.getInt(columnIndex++));
	        table.resetContentsChanged();
    ...
    public WaypointTable fetchWaypoint(long keyId) throws SQLException {
        ...
        Cursor cursor =
        	dbAdapter.getSqldb().query(true, TABLE_NAME, new String[] {DbConst.KEY_ID, DbConst.TRIP_ID, DbConst.NAME,
            		DbConst.WHEN, DbConst.ADDRESS, DbConst.ALTITUDE, DbConst.LATITUDE, DbConst.LONGITUDE, DbConst.COMMENT, DbConst.TRAIL, DbConst.LAST_CRUMB, DbConst.CRUMB_NUM}, DbConst.KEY_ID + "=" + keyId, null,
                        null, null, null, null);
        if (cursor == null) {
        	return null;
        }
